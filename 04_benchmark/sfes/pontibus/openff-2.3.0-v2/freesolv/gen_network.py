...
def get_water_settings():
    ...
    settings.solvent_forcefield_settings.forcefields = [
        ...
        pathlib.Path("../fb-fit-v2-single-mean-eps-with-am1bcc.offxml").read_text(),
        "tip3p.offxml",
    ]
    settings.vacuum_forcefield_settings.forcefields = [
        pathlib.Path("../fb-fit-v2-single-mean-eps-with-am1bcc.offxml").read_text(),  # as above
    ]
...
def run(ligands: pathlib.Path, fsolv: pathlib.Path):
    fsolv_data = pandas.read_csv(fsolv)
...
if __name__ == "__main__":
    ligands = pathlib.Path("../mnsol_freesolv_highrmse_openff-gnn-am1bcc-0.1.0-rc.3.pt_openff-nagl.sdf")
    fsolv = pathlib.Path("../../openff-2.2.1-oe/sage-fsolv-test-v1.csv")
    run(ligands, fsolv)
