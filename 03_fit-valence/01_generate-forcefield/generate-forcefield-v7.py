...
def split_msm_angles(angle_handler):
    ...
    angle_handler.add_parameter(parameter=a7a, before=a7.smirks)
...
def fix_problematic_angles(angle_handler):
    ...
    a21 = angle_handler.get_parameter({"id": "a21"})[0]
    assert a21.smirks == "[#1:1]-[#7X3$(*~[#6X3,#6X2,#7X2+0]):2]-[*:3]"
    a21a = angle_handler.get_parameter({"id": "a21a"})[0]
    a21b = copy.deepcopy(a21)
    a21b.id = "a21b"
    a21b.smirks = "[#1:1]-[#7X3$(*~[#6X3](~[#7])~[#7]):2]~[#6:3]"
    angle_handler.add_parameter(parameter=a21b, after=a21a.smirks)

    a21c = copy.deepcopy(a21)
    a21c.id = "a21c"
    a21c.smirks = "[#1:1]-[#7X3$(*-[#6X3]):2]-[#1:3]"
    angle_handler.add_parameter(parameter=a21c, after=a21b.smirks)
    ...
    # == new v7 ==
    # add new a19b before a19 to capture general N bound to O=SX4=O
    # right now, a18d is unexpectedly capturing this
    # with adjacent 5-membered rings
    a19b = copy.deepcopy(a19)
    a19b.id = "a19b"
    a19b.smirks = "[*:1]~[#7X3:2]-[#16X4:3](~[#8X1])~[#8X1]"
    angle_handler.add_parameter(parameter=a19b, before=a19.smirks)

    # and a new a19c for N-S*
    a19c = copy.deepcopy(a19)
    a19c.id = "a19c"
    a19c.smirks = "[#7X3:1]-[#16X4:2](~[#8X1])~[#8X1:3]"
    angle_handler.add_parameter(parameter=a19c, after=a19b.smirks)

    # add external 5-membered ring angle
    a41 = angle_handler.get_parameter({"id": "a41"})[0]
    a41b = copy.deepcopy(a41)
    a41b.id = "a41b"
    a41b.smirks = "[*:1]!@[*;r5:2]1@[*;r5:3]@[*;r5]@[*;r5]@[*;r5]~1"
    angle_handler.add_parameter(parameter=a41b, before=a41.smirks)

    # add external 4-membered ring angle
    a7a = angle_handler.get_parameter({"id": "a7a"})[0]
    a7b = copy.deepcopy(a7a)
    a7b.id = "a7b"
    a7b.smirks = "[*:1]!@[r4:2]1-;@[r4:3]-;@[r4]~[*]~1"
    angle_handler.add_parameter(parameter=a7b, after=a7a.smirks)
...
def move_r4_angles_to_end(angle_handler):
    ...
    # move a13a and a14 after a41a
    a41a = angle_handler.get_parameter({"id": "a41a"})[0]
    a13a = angle_handler.get_parameter({"id": "a13a"})[0]
    a14 = angle_handler.get_parameter({"id": "a14"})[0]
    angle_handler._parameters.remove(a13a)
    angle_handler._parameters.remove(a14)
    angle_handler.add_parameter(
        parameter=a13a,
        after=a41a.smirks
    )
...
@click.option(
    ...
    "-o",
    "output_path",
    default="output/initial-force-field-v7.offxml",
