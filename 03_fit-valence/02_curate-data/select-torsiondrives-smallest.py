...
def main(
    ...
    print(f"Loaded {len(all_parameter_ids)} torsion ids")
    ...
    print(f"Loaded dataset of {len(df)} records")
    # Filter out exclude cmiles
    df = df[~df.cmiles.isin(exclude_cmiles)]
    print(f"Filtered dataset to {len(df)} records after excluding bad cmiles")
    ...
    if exclude_ids:
        ...
        print(f"Filtered dataset to {len(df)} records after excluding bad QCA IDs")
    ...
    print(f"Filtered cmiles to {len(filtered_cmiles)} CMILES")
    df = df[
    ...
    print(f"Filtered to {table_subset.count_rows()} records")
    ...
    for parameter_id, torsion_ids in tqdm.tqdm(
        ...
        if len(torsion_ids) <= n_records:
            ...
            print(f"Selected {len(torsion_ids)} for {parameter_id}")
    ...
    for parameter_id, _ in tqdm.tqdm(
        ...
        print(f"Selected {len(additional_ids)} for {parameter_id}")


    print(f"Selected {len(selected_ids)} torsions")
    ...
    for parameter_id, count in parameter_counts.items():
        if not count:
            print(parameter_id)
    print("Parameters with 1-5 counts:")
    for parameter_id, count in parameter_counts.items():
    ...
    print(f"Wrote {len(torsiondrive_results)} records to {output_file}")
